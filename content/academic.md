---
title: "学业"
date: 2024-04-07
draft: false
hidemeta: true
---

{{< course title="常微分方程（刘保平）" >}}

  {{< category title="笔记" >}}
    <li class="pdf-item"><a href="/docs/5 常微分方程/笔记/main.pdf">完整笔记</a></li>
  {{< /category >}}

  {{< category title="作业" >}}
    {{< batch-pdf base="/docs/5 常微分方程/作业" file_prefix="作业" name="作业" count="10" >}}
  {{< /category >}}

{{< /course >}}

{{< course title="应用数学导论（胡俊）" >}}

  {{< category title="上机作业" >}}
    {{< batch-pdf base="/docs/6 应数导/上机作业" dir_prefix="作业" file_prefix="报告" name="上机作业" count="3" >}}
  {{< /category >}}

  {{< category title="书面作业" >}}
    {{< batch-pdf base="/docs/6 应数导/书面作业" file_prefix="作业" name="书面作业" count="7" >}}
  {{< /category >}}

{{< /course >}}

{{< course title="数值分析（李铁军）" >}}

  {{< category title="笔记" >}}
    <li class="pdf-item"><a href="/docs/7 数值分析/笔记/main.pdf">完整笔记</a></li>
  {{< /category >}}

  {{< category title="上机作业" >}}
    {{< batch-pdf base="/docs/7 数值分析/上机作业" dir_prefix="作业" file_prefix="报告" name="上机作业" count="4" >}}
  {{< /category >}}

  {{< category title="书面作业" >}}
    {{< batch-pdf base="/docs/7 数值分析/书面作业" file_prefix="作业" name="书面作业" count="22" >}}
  {{< /category >}}

{{< /course >}}

{{< course title="科学计算中的量子算法（安冬）" >}}

  {{< category title="作业" >}}
    {{< batch-pdf base="/docs/8 科学计算中的量子算法/作业" file_prefix="作业" name="作业" count="3" >}}
  {{< /category >}}

{{< /course >}}